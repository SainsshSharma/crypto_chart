...
const Heading = () => {
  ...
  return (
    <div className="headingContainer">
      ...
      {
        <div className={data?(Number(data.ath_change_percentage) >=0 ? `profit gain`:`profit loss`):""}>
        
        {data? Number(data.price_change_24h).toLocaleString():"Loading"} ( {data?Number(data.ath_change_percentage).toLocaleString()+"%":"Loading"} )
      </div>
      }
    </div>
  );
